...
// Each order: { id, name, location, table, items, timestamp }
let orders = [];
let nextOrderId = 1;
...
// Serve staff dashboard HTML page
...
app.get('/orders', (req, res) => {
  ...
  // Map orders with calculated status
  const ordersWithStatus = orders.map(order => {
    const orderTime = new Date(order.timestamp);
    const diffMinutes = (now - orderTime) / 60000;
    let status = "";
    if (diffMinutes < 2) status = "In List";
    else if (diffMinutes < 7) status = "Preparing";
    else status = "Ready";
